...
const TASK_PROMPT = {
  ...
  research: (name, instruction, selection, ctx) =>
    ...
    `用知识库(Read/Grep) + WebSearch + 白名单域 WebFetch 收集**带来源**的事实素材，输出一段可引用的调研小结：每条事实后跟出处（KB src 或 URL）。只收事实、标不确定项，不下结论、不替我写正文。${ctx || ''}`,
};
...
export async function startTask(name, type, instruction, selection, model, onEvent) {
  ...
  // 调研需要工具（读库/白名单网页），非 agentic 模型（deepseek）做不了——如实拒绝，别空转。
  if (type === 'research' && !isAgentic(model || CONFIG.DEFAULT_MODEL)) {
    onEvent({ type: 'error', message: '调研需要 Claude（要读知识库/网页的工具）；deepseek 只能做补写。把模型切回 claude 再派调研。' });
    return;
  }
  ...
  const { contextBlock } = assemble(name, { agentic });
  const sessionKey = `task:${id}`;
  const session = agentic ? getSession(sessionKey, mid, kind) : null;
  onEvent({ type: 'started', id, kind: agentic ? kind : 'completion' });
  let out = '';
  try {
